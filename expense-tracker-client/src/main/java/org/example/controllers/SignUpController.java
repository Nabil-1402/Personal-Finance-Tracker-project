...
public class SignUpController {
        ...
        signUpView.getRegisterButton().setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                ...
                if(postCreateAccountStatus){
                 Utilities.showAlertDialog(Alert.AlertType.INFORMATION,"Successfully created new account!");
                 new LoginView().show();
                }else{
                    Utilities.showAlertDialog(Alert.AlertType.ERROR,"Failed to create new account...");
                }
            }
        });
    ...
    private boolean validateInput(){
        ...
        //entered email doesnt already exist in our database
        if(SqlUtil.getUserByEmail(signUpView.getUsernameField().getText()) != null){
            return false;
        }
