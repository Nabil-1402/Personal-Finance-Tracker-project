...
public class DashbordView {
    ...
    public DashbordView(String email){
        this.email = email;
        loadingAnimationPane = new LoadingAnimationPane(Utilities.APP_WIDTH, Utilities.APP_HEIGHT);
    ...
    public Scene createScene(){
        ...
        mainContainerWrapper.getStyleClass().addAll("dashbord-padding");
        VBox.setVgrow(mainContainerWrapper, Priority.ALWAYS);

        HBox balanceSummaryBox = createBalanceSummaryBox();
        GridPane contentGridPane = createConetentGridPane();
    ...
    private GridPane createConetentGridPane(){
    ...
    private VBox createRecentTransactionsVBox(){
        ...
        recentTransactionLabelandAddBtnBox.getChildren().addAll(recentTransactionLabel, labelAndButtonSpaceRegion, addTransactionButton);
        ...
        recentTransactionsVBox.getChildren().addAll(recentTransactionLabelandAddBtnBox, recentTransactionsScrollPane);
        return recentTransactionsVBox;
    }

    public void resizeTableWidthColumns(){
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                double columnWidth = transactionTable.getWidth() * (0.335);
                monthColumn.setPrefWidth(columnWidth);
                incomeColumn.setPrefWidth(columnWidth);
                expenseColumn.setPrefWidth(columnWidth);
            }
        });
    }
    ...
    public void setCreateCategoryMenuItem(MenuItem createCatagoryMenuItem) {
        this.createCategoryMenuItem = createCatagoryMenuItem;
    }
    ...
    public VBox getRecenetTransactionBox() {
